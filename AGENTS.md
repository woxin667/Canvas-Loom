...
此文件为 Codex (Codex.ai/code) 在处理此存储库中的代码时提供指导。

## 项目概览

这是一个 Obsidian 插件，为 Obsidian Canvas 添加了拆分卡片、复制卡片内容以及为卡片添加数字标记的功能。用户可以根据自定义的分隔符将文本卡片拆分成多个卡片，也可以复制单个或多个卡片的内容到剪贴板（支持按位置排序和按标记排序），还可以为卡片添加数字、文字或 emoji 标记以标识状态或优先级。
...
- **`src/services/`** - 服务层，处理业务逻辑
  ...
  - `BadgeService.ts` - 标记管理服务
  - `ContentService.ts` - 内容处理服务
- **`src/presentation/`** - 表现层，UI组件和命令处理
  - `commands/` - 命令处理器（拆分、复制、标记操作）
  - `modals/` - 模态框组件（单卡片属性查看器、多卡片属性查看器）
  - `styles/` - 样式管理器
...
3. 被触发时，根据情况提供以下功能：
   ...
   - 按标记顺序复制多个卡片的内容到剪贴板（格式：[标记] 内容）
   - 为卡片添加/编辑/移除数字标记
   - 查看和编辑卡片属性（尺寸、位置等）
4. 新卡片会以固定的尺寸创建，并垂直排列在原始卡片的右侧
5. 标记信息通过 Canvas API 持久化存储在 .canvas 文件中
6. 标记显示通过 CSS 伪元素实现，支持数字、文字和 emoji 类型
...
- **[功能：按标记顺序复制内容](./docs/功能-按标记顺序复制内容.md)**
- **[功能：新 Canvas 卡片的尺寸调整](./docs/功能-Canvas卡片尺寸调整.md)**
- **[功能：为 Canvas 卡片添加/移除数字标记](./docs/功能-卡片标记.md)**
- **[功能：查看和编辑卡片属性](./docs/功能-查看和编辑卡片属性.md)**
...
- 每次对项目进行改动后，都应主动询问是否需要更新或创建 `docs` 目录下的相关功能的技术文档，以确保文档的同步和准确性。

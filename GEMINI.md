...
这是一个 Obsidian 插件，允许用户根据自定义的分隔符，将 Obsidian Canvas 中的文本卡片分割成多个卡片。该插件会为 Canvas 中的文本节点新增一个右键菜单选项，可将卡片内容分割成多个新卡片，并将它们放置在原始卡片的右侧。
...
### 关键实现细节
1. 插件监听 'canvas:node-menu' 事件以新增右键菜单项
2. 被触发时，它会使用用户定义的分隔符来分割 Canvas 文本节点的内容
3. 新卡片会以固定的尺寸创建，并垂直排列在原始卡片的右侧
4. 设置会使用 Obsidian 内置的数据加载/保存机制进行持久化

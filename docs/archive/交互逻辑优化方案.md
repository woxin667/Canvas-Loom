...
右键菜单中只放“一键复制”和“一键拼合”，用户可以在设置中选择一键的逻辑是“按位置”或“按徽章顺序”
...
拖拽只在"手动"模式下生效
顶部选"位置"或"徽章"时，卡片按该规则自动排列，用户看到顺序但不能拖。切到"手动"时拖拽才激活——而且此时可以以当前顺序（位置或徽章）为起点继续调整，这比现在"一进手动就要从头拖"好用得多。
...
- 旧复制命令 `按位置复制/按徽章复制/手动排序复制` 也从右键菜单移除，命令面板是否保留可后续决定
...
- 验收标准：从 `位置` 或 `徽章` 切到 `手动` 时，列表顺序保持当前结果；拖拽后输出结果同步变化
